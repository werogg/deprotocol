...
from deprotocol.settings import TOR_BINARIES_DOWNLOAD_DIR
    ...
    def download_and_install():
        ...
        if os.path.isfile(TOR_BINARIES_DIR):
            Logger.get_logger().warning(
                'A tor installation was found in your system, if DeProtocol is not working please delete data/bin '
                'folder '
            )
            return

        if not os.path.isfile(TOR_BINARIES_DOWNLOAD_DIR):
            ...
            with open(TOR_BINARIES_DOWNLOAD_DIR, 'wb') as f:
                f.write(response.content)

        try:
            with tarfile.open(TOR_BINARIES_DOWNLOAD_DIR, 'r:gz') as tar:
